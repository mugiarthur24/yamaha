    ...
    public function create(){
        if ($this->ion_auth->logged_in()) {
            ...
            if (!$this->ion_auth->in_group($level)) {
                ...
                $data['page'] = 'admin/setting/tambah-v';
                // pagging setting
                $this->load->view('admin/dashboard-v',$data);
            }
        }else{
            ...
            if (!$this->ion_auth->in_group($level)) {
                ...
                if (!empty($_FILES["logopt"]["tmp_name"])) {
                    ...
                    if (!$this->upload->do_upload('logopt')){
                        ...
                        redirect(base_url('index.php/admin/setting/create'));
                    }
                    else{
    ...
    public function update($id){
        if ($this->ion_auth->logged_in()) {
            ...
            if (!$this->ion_auth->in_group($level)) {
                ...
                $infopt = $this->detail_data('info_pt','id_info_pt',$id);
                if ($infopt ==TRUE) {
                    $getuser= $this->ion_auth->user()->row();
                    $post = $this->input->post();
                    $data = array(
                    ...
                    if (!empty($_FILES["logopt"]["tmp_name"])) {
                        ...
                        if (!$this->upload->do_upload('logopt')){
                            ...
                            $file = $this->Admin_m->cek_pt($infopt->id_info_pt)->row('logo_pt');
                            if ($file != "logo.png") {
                    ...
                    $this->Admin_m->update('info_pt','id_info_pt',$infopt->id_info_pt,$data);
